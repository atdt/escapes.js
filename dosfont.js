...
for(i = 0; i < 16; i++) {
    ...
    row = letter[i];
    for (j = 8; j >= 0; j--) {
        arr[i].push.apply(arr[i], row & 1 ? ['X','X','X'] : [' ', ' ', ' ']);
        // arr[i].push(row & 1 ? 0 : 1);
        row >>= 1;
    }
}
...
// Creates a canvas element with a null namespaceURI.

    canvas = document.createElement('canvas');
    canvas.width = 8;
    canvas.height = 16;
    context = canvas.getContext('2d');
    bitmap = context.createImageData(8, 16);
    pixel_array = bitmap.data;

    function rasterize(charcode, foreground, background) {
        var letterform = ansi.data[charcode];
